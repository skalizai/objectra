...
Originally shipped as a one-off Excel workbook for ShiftX (see the prompt below section 27a) — this section documents the real, permanent app feature built from it. PMs register newly requested items with a Dev/Functional/Fiori **effort-in-days** estimate (no cost/rate figures anywhere — that layer was removed post-launch at the user's explicit request, see 27b), send a batch to the client for approval by email, and once approved, promote them into the real Objects register. New **Backlog** tab in the project workspace (`components/projects/workspace-tabs.tsx`), visible only to org_admin/PM/technical_lead — unlike Objects/Pipeline/Support, no other role ever sees it.

**Data model** (`supabase/migrations/0034_backlog_items.sql`, simplified by `0037_backlog_remove_cost.sql`):

- `backlog_items` — `item_no` (auto-generated `<PROJECT4>-BL-<seq5>` via `backlog_sequences` + `generate_backlog_no()`, same pattern as `ticket_no`), `company_code`/`module` (existing org picklists), `dev_type` (**new** picklist type — Settings → "Backlog item types"), `complexity` (existing `complexity` picklist, auto-banded from Dev Days but overridable), `dev_days`/`fiori_days`/`func_days` (plain numeric effort-in-days, no hours/cost derivation), `status` (`registered → sent_for_approval → approved/rejected/on_hold → moved_to_objects`, fixed enum not a picklist), `cr_no`, `sent_for_approval_at`, `approval_date`, `converted_object_id` (nullable FK to `objects`, set by the move action).
...
1. PM registers items from the Backlog tab (`AddBacklogItemButton`/`BacklogItemDrawer`, `lib/actions/backlog.ts::createBacklogItem`/`updateBacklogItem`) — Dev/Functional/Fiori days are the only effort inputs; Complexity auto-bands from Dev Days but is overridable.
2. PM multi-selects `registered` items in the table (which shows Dev days/Func days/Fiori days columns), enters a CR number, and sends — `sendForApproval()` bulk-transitions to `sent_for_approval` (guarded to only affect rows still actually `registered`) and fires `notifyBacklogApprovalRequest()` (`lib/email/notify-backlog.ts`), one email per client recipient (`project_members` role=`client`, same join `weekly-digest.ts` uses — no separate "client contact" concept), listing every selected item's day breakdown plus a batch total-days figure, gated on a new `notification_settings.backlog_emails_enabled` toggle, logged to `email_log` as `backlog_approval_request`.
...
### 27b. Cost layer removed (added post-launch)
...
`backlog_items` also gained a **Package** field (`0038_backlog_package.sql`): a fixed 4-value set (`Package 1`–`Package 4`, `BACKLOG_PACKAGES` in `lib/types/database.ts`) — not an org-editable picklist, since the values themselves were specified as fixed, same reasoning as `BacklogItemStatus`. Selectable in the create/edit forms and filterable (with its own column) on the register. All 31 bulk-imported items were tagged `Package 1` in the same migration (matched by project + exact description, so only those rows are touched). A second bulk import (34 items from the PTW/EAM cost workbook's main tab — Module, Description, Dev/Functional days only, no LOB/Type/Go-Live-Critical in that source) followed the same safe-project-match pattern, tagged `Package 2`.

### 27d. Backlog dashboard (added post-launch)

`components/backlog/backlog-dashboard.tsx`, rendered above the register table/toolbar in `backlog-register.tsx`: a prominent **Package selector** (`All packages` | `Package 1`–`4`) driving six cards — **By module**, **By LOB**, **By type** (donuts, scoped to the selected package, or all items when "All packages"), **By package** (donut, always shows every item's package split regardless of the selector — scoping it to one package would just be a single 100% slice), **Effort days by type** (bar — Dev/Fiori/Functional day totals, fixed per-type colors, scoped like the first three donuts), and **Total days by module** (bar — sum of Dev+Fiori+Functional days per module, single-hue magnitude bar mirroring `ticket-module-bar.tsx`, also scoped). The Package selector is shared state with the table's own package filter, so picking a package scopes both the charts and the list below it in one control. Donuts use `components/backlog/backlog-category-donut.tsx` (a generic reusable version of `TicketStatusDonut`'s donut+legend shape); the two bar cards (`backlog-effort-type-bar.tsx`, `backlog-module-days-bar.tsx`) mirror `TicketCriticalityBar`/`TicketModuleBar`.
...
Two more sections render below the existing charts, **always** — not gated behind a project being selected, so "All projects" (the default) shows them aggregated across every visible project, same as the top KPI/status/module charts already do:

- **Backlog** — `components/dashboard/project-backlog-summary.tsx` (a thin client wrapper holding its own `packageFilter` state so the already-built `BacklogDashboard` can be embedded standalone here) + `getBacklogItems(selectedProjectId)`, where `getBacklogItems()` (`lib/data/backlog.ts`) now takes an *optional* `projectId` — omitted, it drops the `.eq("project_id", ...)` filter entirely and returns every backlog item the viewer's RLS allows across the whole org.
- **Support** — the exact same KPI row + 4 chart cards `/projects/[id]/support` renders (`TicketKpiRow`/`TicketStatusDonut`/`TicketCriticalityBar`/`TicketModuleBar`/`TicketAgingBuckets`/`TicketWorkload`/`TicketTopRaisers`/`TicketDeadlineMonitor`), fed by `getSupportDashboardData(selectedProjectId)` — likewise generalized to an optional `projectId`. The "not in Hypercare/Support phase yet" banner only makes sense for one project, so it's suppressed in "All projects" mode.

The "→ Open Backlog/Support tab" links only render when a single project is selected — there's no one tab to link to from the aggregate view. Neither section duplicates the full interactive table (Backlog register / Tickets table) or the raise/create actions — those stay on their own tabs; the portfolio page links out rather than re-implementing them, to avoid a second copy of the same CRUD surface.

**Fixed in the same pass:** the Backlog dashboard's "By package" donut was originally hardcoded to always show every item's package split regardless of the selector (reasoning: scoping it to one package would trivially be a single 100% slice) — in practice this read as broken ("I picked Package 3 but it still shows Package 1/2 totals"), so it now scopes like every other card. The `DashboardProjectPicker` also switched from a plain server-prop-controlled `<select>` to local optimistic state (`useState` + `useTransition`): the old version visually snapped back to the previous project while `router.push()`'s data fetch was in flight, reading as unresponsive.
...
### 27g. Approve/Reject buttons in the PM approval email (added post-launch)

The user asked for one-click Approve/Reject directly in the PM-approval email instead of requiring the approver to log into Objectra first — important since the approver (`backlog_approver_id`/`pm_id`) may not have an account yet, same "assign before invite" reasoning as everywhere else that resource reference is used.

- **`backlog_approval_tokens`** (new table, `0040_backlog_approval_action_links.sql`) — one row per PM-approval send, minted in `notifyBacklogPmApprovalRequest()` right before the email goes out. Mirrors `invitations.token uuid default gen_random_uuid()`; the token itself is the credential, same trust model `/auth/confirm`'s `token_hash` already uses. RLS is enabled with **zero policies** (same convention `email_log` uses) — only the service-role admin client can touch it, and the only caller is the new unauthenticated route below.
- **`app/api/backlog-approval/route.ts`** (new, public) — `GET` never mutates: it validates the token (pending, not expired) and renders an inert HTML confirmation page ("Approve 3 items for ShiftX? [Confirm Approve]"). Only `POST` — reached by clicking that page's button — flips the batch's items `sent_for_approval → approved|rejected`, writes one `backlog_approval_log` row (`actor_id: null`, `note: "Decided via email link"`), and marks the token `used`. The GET/POST split exists specifically because corporate email security scanners (e.g. Microsoft Defender Safe Links) prefetch every link in an inbound email — a bare mutating `GET` would let the scanner itself silently decide the batch before a human ever opens the message.
- **`emails/backlog-approval-email.tsx`** — gained optional `approveUrl`/`rejectUrl` props; when present, renders Approve/Reject buttons above the existing "Open Objectra" link. Only `notifyBacklogPmApprovalRequest()` (the internal PM email) mints a token and passes these — the client-facing `notifyBacklogApprovalRequest()` never does, so no action links ever reach a client.
- Reject is a plain one-click decision from email (no note capture there — a note can still be added afterward from the app's drawer). A batch email acts on its whole batch at once, matching how it was sent as one unit.
