...
`components/backlog/backlog-dashboard.tsx`, rendered above the register table/toolbar in `backlog-register.tsx`: a prominent **Package selector** (`All packages` | `Package 1`–`4`) driving four donut charts (`components/backlog/backlog-category-donut.tsx`, a generic reusable version of `TicketStatusDonut`'s donut+legend shape) — **By module**, **By LOB**, **By type** (scoped to the selected package, or all items when "All packages"), and **By package** (always shows every item's package split, regardless of the selector — scoping it to one package would just be a single 100% slice). The Package selector is shared state with the table's own package filter, so picking a package scopes both the charts and the list below it in one control, per the request.
