...
import { updateOwnName, type SimpleActionState } from "@/lib/actions/settings";

const initialState: SimpleActionState = { error: null, success: false };

export function MyProfileForm({ fullName }: { fullName: string }) {
  ...
  return (
    <div className="rounded-card border border-border bg-surface p-5" style={{ boxShadow: "var(--shadow-card)" }}>
      <h3 className="font-display text-sm font-semibold">Your profile</h3>
